...
class _BarChartRace:
    ...
    def __init__(self, df, filename, orientation, sort, n_bars, fixed_order, fixed_max,
                 ...
                 writer, fig, dpi, bar_kwargs, filter_column_colors, bar_colors = None):
        ...
        self.bar_colors = self.get_bar_colors(bar_colors if bar_colors is not None else cmap)
    ...
    #Updated the get_bar_colors function to receive a dictionary with the column names to give individual colors     
    ...
    def get_bar_colors(self, bar_colors):
        if isinstance(bar_colors, dict):
            # Map the given colors to the DataFrame columns
            mapped_colors = [bar_colors.get(col, 'gray') for col in self.df_values.columns]

            # Convert to numpy array for consistency with existing code
            return np.array(mapped_colors)
        elif isinstance(bar_colors, str):
            from ._colormaps import colormaps
            try:
                bar_colors = colormaps[bar_colors.lower()]
            except KeyError:
                raise KeyError(f'Colormap {bar_colors} does not exist. Here are the '
                            f'possible colormaps: {colormaps.keys()}')
        elif isinstance(bar_colors, colors.Colormap):
            bar_colors = bar_colors(range(bar_colors.N)).tolist()
        elif isinstance(bar_colors, list):
            pass  # bar_colors is already a list
        elif isinstance(bar_colors, tuple):
            bar_colors = list(bar_colors)
        elif hasattr(bar_colors, 'tolist'):
            bar_colors = bar_colors.tolist()
        else:
            raise TypeError('`bar_colors` must be a dictionary of column colors, a string name of a colormap, '
                            'a matplotlib colormap instance, list, or tuple of colors')
...
def bar_chart_race(df, filename=None, orientation='h', sort='desc', n_bars=None, 
                   ...
                   fig=None, dpi=144, bar_kwargs=None, filter_column_colors=False, bar_colors = None):
    '''
    ...
    bcr = _BarChartRace(df, filename, orientation, sort, n_bars, fixed_order, fixed_max,
                        ...
                        shared_fontdict, scale, writer, fig, dpi, bar_kwargs, filter_column_colors, bar_colors)
